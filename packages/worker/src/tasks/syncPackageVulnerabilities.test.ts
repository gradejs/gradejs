...
describe('task / syncPackageVulnerabilities', () => {
  it('should correctly process github advisory db snapshot updates', async () => {
    const fetchAdvisoryDbSnapshotMock = jest
      ...
      .mockImplementationOnce(async () => fs.createReadStream('./.mocks/advisory-database-mock.tar.gz'))
      .mockImplementationOnce(async () => fs.createReadStream('./.mocks/advisory-database-update-mock.tar.gz'));
    ...
    expect(syncedVulnerabilities).toEqual([
      { osvId: 'GHSA-273r-mgr4-v34f', packageName: 'engine.io', packageVersionRange: '>=4.0.0 <4.1.2, >=5.0.0 <5.2.1, >=6.0.0 <6.1.1' },
      { osvId: 'GHSA-6h5x-7c5m-7cr7', packageName: 'eventsource', packageVersionRange: '>=0 <2.0.2' }
    ]);
    ...
    expect(updatedSyncedVulnerabilities).toEqual([
      { osvId: 'GHSA-273r-mgr4-v34f', packageName: 'engine.io', packageVersionRange: '>=4.0.0 <4.1.2, >=5.0.0 <5.2.1' },
      { osvId: 'GHSA-6h5x-7c5m-7cr7', packageName: 'eventsource', packageVersionRange: '>=0 <2.0.2' }
    ]);
  });
});
